...
echo "<b>Scope:</b> Menu: {$item1->nama}, Harga: {$item1->getHarga()}, Stok: {$item1->getStok()}<br>";
...
echo "<b>Type Hinting & Return Types:</b> Total bayar: ".hitungTotal(30000,2)."<br>";

// 9️⃣ Exception Handling
try {
    $stok = 0;
    if($stok<=0) throw new Exception("Stok habis!");
} catch(Exception $e){echo "<b>Exception Handling:</b> Error: ".$e->getMessage()."<br>";}
...
// 14️⃣ CRUD & MVC
class ModelMenu { private $data=[]; public function create($menu){$this->data[]=$menu;} public function read(){return $this->data;} }
$model = new ModelMenu(); $model->create("Es Teh");
...
class DaftarMenu implements IteratorAggregate { private $menu=["Nasi","Ayam","Es Teh"]; public function getIterator(): Traversable{ return new ArrayIterator($this->menu);} }
echo "<b>Iteration:</b><br>";
foreach(new DaftarMenu() as $m){echo "Menu: $m<br>";}

// 17️⃣ Reflection
$ref = new ReflectionClass("MenuItem"); echo "<b>Reflection:</b> Class MenuItem punya property: ";
foreach($ref->getProperties() as $p){echo $p->getName()." ";} echo "<br>";

// 18️⃣ Dependency Injection
class Delivery { private $alamat; public function __construct($alamat){$this->alamat=$alamat;} public function kirim(){echo "<b>Dependency Injection:</b> Kirim ke {$this->alamat}<br>";} }
